/**
 * session-manager.ts — Template resolution and session lifecycle.
 *
 ...
 *
 * Imports from: types.ts, config.ts, agent-static-info.ts, stub-sdk.ts, logs.ts
 */
...
import {logInfo} from "./logs.ts";
export { discoverAgents, getAgentDefinitions, getAgentById, resetAgentCache } from "./agent-static-info";
...
export async function resolvePromptTemplate(
  ...
  let resolved = await readFile(filePath, "utf-8");
  for (let i = 0; i < MAX_PREPROCESS_ITERATIONS; i++) {
    const prev = resolved;
...
export async function* streamSpeech(
  ...
  if (existingSessionId) {
    ...
    // First interaction: create session with the real prompt (no throwaway priming)
    queryOptions.systemPrompt = await buildSystemPrompt(agentId, meetingParticipantDefs);
  }
