...
11. **Personas in `participant-agents/` with template system**: Agent persona files in `participant-agents/` are AI-Agent entry points with YAML frontmatter and undergo template marker resolution (`@include`, `@echo`, `@foreach`, `@foreach-agent`). Shared prompt templates live in `prompts/`. System prompt construction: Participant-Agents get `system-prompt-base-prefix.md` (includes dictionary + fellow participants) + resolved agent file; the Orchestrator-Agent gets `system-prompt-base-prefix.md` + resolved `system-prompt-orchestrator.md`.
...
30. **Actual per-interaction cost tracking**: Costs are tracked per SDK interaction (`ProcessRecord.costUsd`) using the SDK's `total_cost_usd` from result messages. Replaces the earlier `ESTIMATED_COST_PER_CYCLE` constant. Rationale: (a) actual costs are more useful than estimates; (b) per-process granularity survives partial cycles — `pendingCycle` persists process records (with costs) after each phase, so browser refresh or server restart mid-cycle doesn't lose cost data; (c) `totalCostEstimate` on `Meeting` accumulates from committed cycles and, on restart, absorbs costs from any interrupted `pendingCycle`; (d) parallel assessments avoid shared mutable cost state — each `feedMessage`/`streamSpeech` call returns its own `costUsd`.

31. **SDK smoke test validated**: The Agent SDK (`@anthropic-ai/claude-agent-sdk@0.2.62`) was validated via a 25-test smoke test. Five design corrections applied: (a) `tools` option for restriction; (b) `getClaudeProjectDir()` replaces all non-alphanumeric chars; (c) `sessions-index.json` removed; (d) `env` stripping for nested-session prevention; (e) `interrupt()` confirmed as only termination method.
