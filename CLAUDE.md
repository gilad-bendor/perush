...
התקייה הרלבנטית היחידה היא `./PERUSH/` - יש להתעלם מכל התיקיות והקבצים האחרים.
התקייה `./PERUSH/` מכילה קבצי Markdown: כל קובץ מתאר מקטע בטקסט המקראי, ויכיל את הפסוקים שבמקטע, עם הפירוש לפי המילון.
שם הקובץ הוא בפורמט `NNN-חומש-ממקום-למקום-תאור המקטע.rtl.md`
- `NNN` - שם הקובץ מתחיל במספר סידורי בן 3 ספרות - למשל `001` או `002` וכו׳
- `חומש` - למשל `בראשית`
- `ממקום` - הפסוק (פרק+פסוק) הפותח במקטע - למשל `ב יג` כלומר פרק ב׳ פסוק י״ג
- `למקום` - הפסוק (פרק+פסוק) המסיים את המקטע - למשל `ב טו` כלומר פרק ב׳ פסוק ט״ו
