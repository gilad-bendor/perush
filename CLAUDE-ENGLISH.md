...
- The day cycle "...←בוקר←יום←ערב←לילה←בוקר←..." parallels the **Hegelian model** of dialectic and synthesis:
  1. **בוקר** (morning, from בקרה - examination): examination and critique lead to crystallization of dialectic - thesis and antithesis.
  2. **יום** (day): the dialectic generates public discourse, introspection, and searching.
...
- **אש** (fire) = a clear and absolute idea, containing no water (no room for motivation and change).
  **זבח** / **מזבח** (sacrifice/altar) = (related to fire) understanding something as ideal: canonization of a state.
...
3. Check **coherence** on multiple axes:
   ...
   - **Character consistency**: do recurring archetypes (אדם, אישה, נחש, קין...) behave consistently with their established profiles?
   - **Historical overlay continuity**: does the historical mapping maintain chronological order?
4. Check **degrees of freedom**: does the translation follow necessarily from the dictionary, or is there "free imagination"? Be honest - mark weak spots with `<מדרש>` tags or `TODO:`.
5. Flag words that may need deep linguistic analysis (and ask me to research them).
6. Add historical overlay if relevant (inside `<הקבלה-היסטורית>` tags).
...
**When to flag for deep linguistic research**: When a word's root is opaque, contested, or the derived meaning requires careful justification — flag it with a `TODO:` marker. Example:
```
TODO: the word `חדקל` requires linguistic analysis — the root is unclear.
```
After research is complete, the commentary will contain a reference to a linguistic analysis file under `./ניתוחים-לשוניים/`.

# Linguistic Analysis

Biblical Hebrew linguistics is an important area of expertise. You have excellent linguistic ability, but deep linguistic analysis should be performed with other tools I have access to.
...
**Important:** When you encounter a biblical word whose deep meaning needs to be fully extracted to understand the depth of a verse - please ask me to conduct linguistic research on the word and add a reference to the linguistic analysis file from within the commentary.
